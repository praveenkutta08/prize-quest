...
export class PqVoucher extends LitElement {
      ...
      <div class="body">
        ...
        ${!v.redeemed
          ? html`<div class="how">
              ...
              <div class="how__row">
                <span class="how__num">1</span> Show this QR code at any participating outlet.
              </div>
              <div class="how__row">
                <span class="how__num">2</span> Staff scans it and applies the value to your bill.
              </div>
              <div class="how__row">
                <span class="how__num">3</span> Unused balance carries forward until expiry.
              </div>
            </div>`
          : nothing}
        ...
        <div class="actions">
          ${!v.redeemed
            ? html`<button class="cta" @click=${() => this.act("wallet")}>Add to wallet</button>
                <button class="cta cta--ghost" @click=${() => this.act("email")}>
                  Email to me
                </button>`
            : nothing}
    ...
    return html`
      <div class="exp">
        ...
        <div class="exp__nav">
          ...
          <button class="cta cta--ghost" @click=${() => this.act("orders")}>
            View order history
          </button>
        </div>
      </div>
  ...
  private handleCopy = (event?: Event): void => {
    ...
    this.dispatchEvent(
      new CustomEvent("pq-copy", { detail: { value: code }, bubbles: true, composed: true }),
    );
  };

  private act(action: VoucherAction): void {
    this.dispatchEvent(
      new CustomEvent("pq-voucher-action", { detail: { action }, bubbles: true, composed: true }),
    );
  }
