...
const aUser = html`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>`;
const aStar = html`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2l2.4 7.4H22l-6.2 4.5 2.4 7.4L12 16.8 5.8 21.3l2.4-7.4L2 9.4h7.6z"/></svg>`;
const aTag = html`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg>`;
const aPackage = html`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg>`;
const aAward = html`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="8" r="6"/><path d="M15.477 12.89L17 22l-5-3-5 3 1.523-9.11"/></svg>`;
const aLogout = html`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>`;
...
const TILE_TINTS: Record<string, string> = {
  ...
  orange: "--tile-tint: var(--cat-orange, #ff8c2c); --tile-tint-bg: rgba(255,140,44,0.18); --tile-tint-bright: var(--cat-orange-bright, #ffb066);",
  purple: "--tile-tint: var(--cat-purple, #8e47e8); --tile-tint-bg: rgba(142,71,232,0.18); --tile-tint-bright: var(--cat-purple-bright, #b47bff);",
  display: "--tile-tint: var(--arc-display, #ffd93d); --tile-tint-bg: rgba(255,217,61,0.15); --tile-tint-bright: var(--arc-display-bright, #ffee5c);",
  green: "--tile-tint: var(--cat-green, #34d670); --tile-tint-bg: rgba(52,214,112,0.15); --tile-tint-bright: var(--cat-green-bright, #5be389);",
  teal: "--tile-tint: var(--cat-teal, #2dd4bf); --tile-tint-bg: rgba(45,212,191,0.15); --tile-tint-bright: var(--cat-teal-bright, #5eead4);",
  danger: "--tile-tint: var(--arc-danger, #ff4d6d); --tile-tint-bg: rgba(255,77,109,0.12); --tile-tint-bright: #ff8095; opacity: 0.85;",
};

const accountIcon = html`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
...
const promosIcon = html`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
  <path d="M3 11l16-5v12L3 14v-3z" /><path d="M7 18v2a1 1 0 0 1-2 0v-2" /><line x1="19" y1="9" x2="22" y2="9" />
</svg>`;
const ordersIcon = html`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
  <path d="M6 9h12l-1 12H7L6 9z" /><path d="M9 9V6a3 3 0 0 1 6 0v3" />
</svg>`;
const giftIcon = html`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
  <rect x="3" y="9" width="18" height="12" rx="1" /><path d="M12 9v12M3 13h18M12 9S10 4 7.5 5.5 9 9 12 9ZM12 9s2-5 4.5-3.5S15 9 12 9Z" />
</svg>`;
const powerIcon = html`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
...
export class TtdHub extends LitElement {
  static override styles = css`
    ...
    .aroot {
      ...
      background:
        ...
        linear-gradient(160deg, var(--arc-bg-deep, #15042e) 0%, var(--arc-bg-base, #1f0b3e) 50%, var(--arc-bg-mid, #2a1454) 100%);
    }
    .scr-head {
      display: flex;
      align-items: center;
  ...
  private renderArcade(): TemplateResult {
    ...
    const tiles: Array<{ name: string; sub: string; icon: TemplateResult; tint: string; hero?: boolean; go: () => void }> = [
      { name: "My Account", sub: "Balance · statements · profile", icon: aUser, tint: "blue", go: () => {} },
      { name: "Prize Quest", sub: "Campaigns · prizes · claims", icon: aStar, tint: "pink", hero: true, go: () => this.#go("/") },
      { name: "Promotions", sub: "Free play · multipliers · offers", icon: aTag, tint: "orange", go: () => {} },
      { name: "Order History", sub: "Past prizes · tracking · receipts", icon: aPackage, tint: "purple", go: () => this.#go("/orders") },
      { name: "Tier Status", sub: "Platinum · benefits · next tier", icon: aAward, tint: "display", go: () => {} },
      { name: "Sign Out", sub: "End session · return to attract", icon: aLogout, tint: "danger", go: this.#signOut },
    ];
    return html`
      <div class="aroot">
        <div class="scr-head">
      ...
      <div class="root">
        <div class="top">
          ...
          <div class="right">
            ...
            <button class="signout-btn" type="button" title="Sign out" aria-label="Sign out" @click=${this.#signOut}>
              ${powerIcon}
            </button>
          </div>
        </div>
        <div class="grid">
          ...
          <div class="row row--ours">
            ...
            <button class="tile tile--ours tile--hero" type="button" @click=${() => this.#go("/")}>
              ${this.claimableCount > 0 ? html`<span class="badge">${this.claimableCount} Ready</span>` : ""}
